...
import { GET_TEACHER_BY_PATH } from '@/lib/queries'
import { DrupalTeacher } from '@/lib/types'
...
interface TeacherByPathData {
  route: {
    entity: DrupalTeacher
  } | null
}

async function getTeacher(path: string): Promise<DrupalTeacher | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<TeacherByPathData>({
      query: GET_TEACHER_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/teachers/${slug.join('/')}`
  const item = await getTeacher(path)

  if (!item) {
    return { title: 'Teacher Not Found | Riverside K-12 Academy' }
  }

  return {
    title: `${item.title} | Riverside K-12 Academy`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).office && (
                    <div>
                      <dt className="text-sm text-gray-500">Office/Room</dt>
